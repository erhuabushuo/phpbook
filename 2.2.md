...
对象是一个复杂的结构体（zend_object_value结构体），不仅存储属性的定义、属性的值，还存储着访问权限、方法等信息。内核中定义了以下组合宏让我们方便的操作对象：OBJ_HANDLE, 返回标识符, OBJ_HT 返回表, OBJCE 返回类定义, OBJPROP HashTable 属性, 以及 OBJ_HANDLER 自行创建 OBJ_HT 表中的方法. 现在不用担心这些不同宏对象; 它们将会在第10章节详细讨论"PHP4 对象"以及第11章节的"PHP5 对象."
